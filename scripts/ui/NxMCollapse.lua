...
local validateAttributes = function(object)
  ...
  local alwaysBlendEvents = getAttribute(parentNode, "AlwaysCombineSampledEvents")
  ...
  while (index < table.getn(object))do
    if ((string.find(nodeType, "Blend") ~= nil) and (object[index] ~= parentNode))then
      ...
      if (alwaysBlendEvents ~= getAttribute(object[index], "AlwaysCombineSampledEvents")) then
...
local getListOfAttributes = function(object)
  ...
  table.insert(list, getAttribute(parentNode, "AlwaysCombineSampledEvents")) -- 3
...
local createNxM = function(anims, controlParams, attributes, resultPin, xValue, yValue)
  ...
  setAttribute(nodeNxM ..".AlwaysCombineSampledEvents", attributes[3])
